...
public interface ProductSaleRepository extends JpaRepository<ProductSale, Long> {
    ...
    @Query("SELECT new com.iyadsoft.billing_craft_backend.dto.ProfitItemDto(s.productStock.category, s.productStock.brand, s.productStock.productName, COUNT(s.productStock.productno) as qty,  SUM(s.productStock.pprice) as pprice, SUM(s.sprice) as sprice, SUM(s.discount) as discount ) "
            ...
            "FROM ProductSale s " +
            "WHERE s.username = :username AND s.saleType = 'customer' " +
            "AND MONTH(s.date) = MONTH(CURRENT_DATE) AND YEAR(s.date) = YEAR(CURRENT_DATE) GROUP BY s.productStock.category, s.productStock.brand, s.productStock.productName, s.productStock.pprice ")
    ...
    @Query("SELECT new com.iyadsoft.billing_craft_backend.dto.ProfitItemDto(s.productStock.category, s.productStock.brand, s.productStock.productName, COUNT(s.productStock.productno) as qty,  SUM(s.productStock.pprice) as pprice, SUM(s.sprice) as sprice, SUM(s.discount) as discount ) "
            ...
            "FROM ProductSale s " +
            "WHERE s.username = :username AND s.saleType = 'customer' " +
            "AND YEAR(s.date) = :year AND MONTH(s.date) = :month GROUP BY s.productStock.category, s.productStock.brand, s.productStock.productName, s.productStock.pprice ")
    ...
    @Query("SELECT new com.iyadsoft.billing_craft_backend.dto.ProfitItemDto(s.productStock.category, s.productStock.brand, s.productStock.productName, COUNT(s.productStock.productno) as qty,  SUM(s.productStock.pprice) as pprice, SUM(s.sprice) as sprice, SUM(s.discount) as discount ) "
            ...
            "FROM ProductSale s " +
            "WHERE s.username = :username AND s.saleType = 'customer' " +
            "AND s.date BETWEEN :startDate AND :endDate GROUP BY s.productStock.category, s.productStock.brand, s.productStock.productName, s.productStock.pprice ")
    ...
    @Query("SELECT new com.iyadsoft.billing_craft_backend.dto.SupplierDetailsDto(ps.date, ps.customer.cid, CAST(0 AS long), 0.0, SUM(ps.productStock.pprice), 0.0, 0.0, 0.0, 'Vendor', 0.0, 0.0) "
            +
            "FROM ProductSale ps " +
            "WHERE ps.saleType='vendor' AND ps.username = :username AND ps.customer.cName = :supplierName AND ps.date BETWEEN :fromDate AND :toDate "
            +
    ...
    @Query("""
            ...
            FROM ProductSale ps
            WHERE ps.saleType = 'vendor'
            AND ps.username = :username
            AND ps.customer.cName = :supplierName
            AND ps.date < :fromDate
            """)
    Double sumVendorSaleBeforeDate(
            @Param("username") String username,
   ...
   List<ProductSale> findByUsernameAndProductStock_Productno(
            String username,
            String productno
    );

    List<ProductSale> findByCustomer_Cid(String cid);
}
