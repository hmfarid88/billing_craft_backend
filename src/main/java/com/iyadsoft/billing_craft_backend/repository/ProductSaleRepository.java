...
public interface ProductSaleRepository extends JpaRepository<ProductSale, Long> {
    @Query("SELECT new com.iyadsoft.billing_craft_backend.dto.CustomerProductSaleDTO(s.customer.cName, s.customer.phoneNumber, s.customer.address, s.productStock.category, s.productStock.brand, s.productStock.productName, s.productStock.productno, s.productStock.color, s.productStock.sprice, s.discount, s.offer, s.date, s.time, s.customer.cid,  s.productStock.proId, s.username) FROM ProductSale s WHERE s.username=:username AND s.saleType='customer' AND s.date = CURRENT_DATE")
